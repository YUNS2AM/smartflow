...
export const scheduleAPI = {
   ...
   */
  generate: async () => {
    const response = await apiClient.post<ScheduleResult>('/api/schedule/generate');
    return response.data;
  },
   ...
   */
  getResult: async () => {
    const response = await apiClient.get<ScheduleResult>('/api/schedule/result');
    return response.data;
  },
...
export const convertScheduleForGantt = (backend: BackendSchedule, index: number) => {
  ...
  const endDate = new Date(backend.end_time);
